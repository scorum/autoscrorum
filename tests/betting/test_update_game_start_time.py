...
def test_update_game_start_time(wallet_4hf, betting, start, time_shift):
    uuid, _ = betting.create_game(start=start, delay=3600)
...
def test_update_game_start_time_invalid_time(wallet_4hf, betting, start, shift, expected_error):
    uuid, _ = betting.create_game(start=start)
...
def test_update_finished_game_start_time(wallet_4hf, betting):
...
def test_update_game_start_time_invalid_uuid(wallet_4hf, betting):
...
def test_update_game_start_time_not_moderator(wallet_4hf, betting):
...
def test_update_game_start_time_invalid_signing(wallet_4hf, betting):
    uuid, _ = betting.create_game()
    g = {"uuid": uuid, "moderator": betting.moderator, "start_time": fmt_time_from_now(30)}
...
def test_update_start_time_of_game_with_matched_bets_before_start(wallet_4hf, betting, matched_bets):
    game_uuid = betting.create_game_with_bets(matched_bets)
...
def test_update_start_time_of_game_with_pending_bets_before_start(wallet_4hf, betting, pending_bets):
    game_uuid = betting.create_game_with_bets(pending_bets)
    wallet_4hf.update_game_start_time(game_uuid, DEFAULT_WITNESS, fmt_time_from_now(30))
...
def test_update_start_time_of_game_with_bets_added_after_start(wallet_4hf, betting, bets):
...
def test_update_game_start_time_with_bets_few_games(wallet_4hf, betting):
...
def test_update_start_time_restore_bets(wallet_4hf, betting, bets, expected_ops):
    game_uuid = betting.create_game_with_bets(game_start=5, delay=3600, bets=bets, single_block=False)
